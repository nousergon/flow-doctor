...
# editing a file. This repo carried a hand-set `tests-429_passing` badge while
# the suite measured 697 passing, and a hand-set `PyPI-v0.6.0rc3` badge against
# a published v0.16.4.
#
...
# branch already exists (seeded before this script was added), so the
# contents API is enough and no branch is ever created here.
set -euo pipefail
...
# The same measurement the gate uses — coverage.py's own totals over the whole
# flow_doctor package (the `[tool.coverage.run] source` in pyproject.toml),
# read from the .coverage file `coverage run -m pytest` just wrote, not a
# figure re-derived from parsed stdout.
...
if [ "$(gh api "repos/${GITHUB_REPOSITORY}/contents/${DOC}?ref=${BRANCH}" --jq '.content' | base64 --decode)" = "$(cat badge-endpoint.json)" ]; then
  ...
  rm -f coverage-summary.json badge-endpoint.json
  exit 0
fi
...
# Leave the checkout clean — this repo's CI has a step asserting the
# suite left no untracked residue, and these scratch files would trip it.
rm -f coverage-summary.json badge-endpoint.json
