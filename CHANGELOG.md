...
- **A diagnosis on a reasoning model no longer comes back empty and re-files itself as a new report** (alpha-engine-config-I11592). Observed 2026-09-24 ~20:51Z on data-collector: `RouterProvider` called krepis with `max_tokens=2048` on group `low`, which resolved to a reasoning model that spent all 2048 tokens reasoning and returned empty content. krepis logged `llm: EMPTY message.content ... finish_reason='length' completion_tokens=2048 reasoning_tokens=2048` at ERROR from inside the diagnosis, and `FlowDoctorHandler` — on the host's root logger — filed that record as a NEW report, which was diagnosed in turn. The diagnosis itself was lost. Same class as alpha-engine-config-I6917 / I8700 / I6901.

  1. **One budget, one constant.** Every LLM call flow-doctor makes (diagnosis and fix generation, both transports, plus `resolve_router_edge`'s default) now uses `flow_doctor.core.constants.LLM_MAX_TOKENS = 16000` instead of scattered 2048 / 4096 literals. The number is the one the fleet already measured for `low` under I8700 (p99 reasoning draw 13089 + p95 answer 1105 = 14194; the Think Tank `sweep` tier on `low` was raised to 16000 on it), not a new guess. A ceiling is billed only when drawn. `tests/test_llm_budget_and_self_capture.py` fails if any `max_tokens=` in the package is anything other than the constant.
  2. **flow-doctor never files its own diagnosis call's log records.** `FlowDoctor._run_diagnosis` runs the provider inside `own_llm_call_scope()` (a `ContextVar`, so per-thread / per-task), and `FlowDoctorHandler.emit` drops records emitted inside it — whatever logger they come from. The existing self-exclusion was by logger name (`flow_doctor.*`) and could not see a `krepis.llm` record. A record the host logs on any other thread while a diagnosis is in flight is still captured, and every excluded record still reaches the host's other handlers.
  3. **Empty content degrades instead of fabricating.** Both providers raise `EmptyDiagnosisResponse` (naming `finish_reason`, completion and reasoning tokens) rather than parsing `''` into a 0.3-confidence `CODE` diagnosis. `_run_diagnosis` already catches it: the report is filed and dispatched without a diagnosis, with `report.diagnosis_error` saying why. The router path records the billed spend before raising.
